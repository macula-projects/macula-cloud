    ...
    public static List<AppClusterServerStateWrapVO> wrapToAppClusterServerState(
        ...
        for (ClusterUniversalStatePairVO stateVO : list) {
            ...
            if (mode == ClusterStateManager.CLUSTER_SERVER) {
                ...
                map.computeIfAbsent(serverId,
                    v -> new AppClusterServerStateWrapVO().setId(serverId).setIp(ip).setPort(serverStateVO.getPort())
                        .setState(serverStateVO).setBelongToApp(true).setConnectedCount(
                            serverStateVO.getConnection().stream().mapToInt(ConnectionGroupVO::getConnectedCount)
                                .sum()));
        ...
        for (ClusterUniversalStatePairVO stateVO : list) {
            ...
            if (mode == ClusterStateManager.CLUSTER_CLIENT) {
                ...
                map.computeIfAbsent(serverId,
                    v -> new AppClusterServerStateWrapVO().setId(serverId).setIp(serverIp).setPort(serverPort)
        ...
        for (ClusterUniversalStatePairVO stateVO : list) {
            ...
            if (mode == ClusterStateManager.CLUSTER_CLIENT) {
                ...
                map.computeIfAbsent(clientId,
                    v -> new AppClusterClientStateWrapVO().setId(clientId).setIp(ip).setState(clientStateVO)
    ...
    public static List<ClusterGroupEntity> wrapToClusterGroup(List<ClusterUniversalStatePairVO> list) {
        ...
        for (ClusterUniversalStatePairVO stateVO : list) {
            ...
            if (mode == ClusterStateManager.CLUSTER_SERVER) {
                ...
                map.computeIfAbsent(serverAddress,
                    v -> new ClusterGroupEntity().setBelongToApp(true).setMachineId(ip + '@' + stateVO.getCommandPort())
        ...
        for (ClusterUniversalStatePairVO stateVO : list) {
            ...
            if (mode == ClusterStateManager.CLUSTER_CLIENT) {
                ...
                ClusterGroupEntity group = map.computeIfAbsent(targetServer,
                    v -> new ClusterGroupEntity().setBelongToApp(true).setMachineId(targetServer).setIp(targetServer)
                        .setPort(targetPort));
