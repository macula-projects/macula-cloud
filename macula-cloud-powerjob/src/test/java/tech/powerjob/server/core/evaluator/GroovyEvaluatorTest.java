...
public class GroovyEvaluatorTest {
    ...
    public void testSimpleEval3() {
        Object res =
            groovyEvaluator.evaluate("var res = new groovy.json.JsonSlurper().parseText(context.k3); res == false;",
                SIMPLE_CONTEXT);
    ...
    public void testComplexEval1() {
        // array
        Object res =
            groovyEvaluator.evaluate("var res = new groovy.json.JsonSlurper().parseText(context.array) ; res[0] == 1;",
                COMPLEX_CONTEXT);
        Assertions.assertEquals(true, res);
        // map
        res = groovyEvaluator.evaluate(
            "var map = new groovy.json.JsonSlurper().parseText(context.map); var e1 = map.e1; e1.value ",
            COMPLEX_CONTEXT);
        Assertions.assertEquals(1, res);
        // object
        res = groovyEvaluator.evaluate(
            "var e3 = new groovy.json.JsonSlurper().parseText(context.obj); var e1 = e3.sub.sub; e1.value ",
            COMPLEX_CONTEXT);
        Assertions.assertEquals(1, res);
    }
