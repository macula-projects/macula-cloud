...
public class CronTimingStrategyHandlerTest {
    ...
    private static final List<String> CRON_LIST =
        Lists.list("0 0 10,11,12 * * ?", "0 0/30 9-17 * * ?", "0 0 12 ? * WED", "0 0 12 * * ?", "0 15 11 ? * *",
            "0 15 11 * * ?", "0 15 11 * * ? *", "0 15 11 * * ? 2088", "0 * 14 * * ?", "0 0/5 14 * * ?",
            "0 0/5 14,18 * * ?", "0 0-5 14 * * ?", "0 10,44 14 ? 3 WED", "0 15 11 ? * MON-FRI", "0 15 11 15 * ?",
            "0 15 11 L * ?", "0 15 11 ? * 6L", "0 15 11 ? * 6L 2087-2088", "0 15 11 ? * 6L 2087-2088",
            "0 15 11 ? * 6#3", "0 0 0 1 1 ? 2088");
    ...
    public void compareToQuartzCron() {
        ...
        for (String cron : CRON_LIST) {
            ...
            while (referenceTime != null && count < 50) {
                ...
                log.info("cron:'{}',reference time:{},quartz result:{},new result:{}", cron, referenceTime, quartzRes,
                    newRes);
    ...
    public void test01() {
        ...
        Long nextTriggerTime = cronTimingStrategyHandler.calculateNextTriggerTime(referenceTime, cron,
            start.toEpochSecond(ZoneOffset.of("+8")) * 1000, end.toEpochSecond(ZoneOffset.of("+8")) * 1000);
        Assertions.assertEquals("2088-05-01 11:15:00",
            DateFormatUtils.format(nextTriggerTime, OmsConstant.TIME_PATTERN));
    }
