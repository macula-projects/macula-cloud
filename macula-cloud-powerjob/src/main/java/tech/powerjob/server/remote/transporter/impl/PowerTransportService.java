...
public class PowerTransportService
    ...
    private void initRemoteFrameWork(String protocol, int port) {
        ...
        log.info("[PowerTransportService] find Actor num={},names={}", beansWithAnnotation.size(),
            beansWithAnnotation.keySet());

        Address address = new Address().setHost(NetUtils.getLocalHost()).setPort(port);
        EngineConfig engineConfig =
            new EngineConfig().setServerType(ServerType.SERVER).setType(protocol.toUpperCase()).setBindAddress(address)
        ...
        this.engines.add(re);
        this.protocolName2Info.put(protocol,
            new ProtocolInfo(protocol, address.toFullAddress(), engineOutput.getTransporter()));
    }
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        for (String protocol : activeProtocols.split(OmsConstant.COMMA)) {
            try {
                ...
                log.error(
                    "[PowerTransportService] initialize protocol[{}] failed. If you don't need to use this protocol, you can turn it off by 'oms.transporter.active.protocols'",
                    protocol);
    ...
    private int parseProtocolPort(String protocol) {
        ...
        if (StringUtils.isEmpty(portStr)) {
            throw new IllegalArgumentException(
                String.format("can't find protocol config by key: %s, please check your spring config!", key1));
        }
