...
public class WorkflowService {
    ...
    public Long saveWorkflow(SaveWorkflowRequest req) {
        ...
        if (wfId == null) {
            ...
            Long finalWfId = wfId;
            wf = workflowInfoRepository.findById(wfId)
                .orElseThrow(() -> new IllegalArgumentException("can't find workflow by id:" + finalWfId));
        }
        ...
        if (TimeExpressionType.FREQUENT_TYPES.contains(req.getTimeExpressionType().getV())) {
            ...
            Long nextValidTime = timingStrategyService.calculateNextTriggerTimeWithInspection(
                TimeExpressionType.of(wf.getTimeExpressionType()), wf.getTimeExpression(), lifeCycle.getStart(),
                lifeCycle.getEnd());
    ...
    private String validateAndConvert2String(Long wfId, PEWorkflowDAG dag) {
        ...
        for (PEWorkflowDAG.Node node : dag.getNodes()) {
            WorkflowNodeInfoDO nodeInfo = workflowNodeInfoRepository.findById(node.getNodeId())
                .orElseThrow(() -> new PowerJobException("can't find node info by id :" + node.getNodeId()));
            // 更新工作流 ID
    ...
    public long copyWorkflow(Long wfId, Long appId) {
        ...
        if (!CollectionUtils.isEmpty(dag.getNodes())) {
            ...
            for (PEWorkflowDAG.Node node : dag.getNodes()) {
                ...
                WorkflowNodeInfoDO originNode = workflowNodeInfoRepository.findById(node.getNodeId()).orElseThrow(
                    () -> new IllegalArgumentException("can't find workflow Node by id: " + node.getNodeId()));
    ...
    public Long runWorkflow(Long wfId, Long appId, String initParams, Long delay) {
        ...
        log.info("[WorkflowService-{}] try to run workflow, initParams={},delay={} ms.", wfInfo.getId(), initParams,
            delay);
        Long wfInstanceId =
            workflowInstanceManager.create(wfInfo, initParams, System.currentTimeMillis() + delay, null);
        if (delay <= 0) {
    ...
    public List<WorkflowNodeInfoDO> saveWorkflowNode(List<SaveWorkflowNodeRequest> workflowNodeRequestList) {
        ...
        for (SaveWorkflowNodeRequest req : workflowNodeRequestList) {
            ...
            if (req.getId() != null) {
                workflowNodeInfo = workflowNodeInfoRepository.findById(req.getId())
                    .orElseThrow(() -> new IllegalArgumentException("can't find workflow Node by id: " + req.getId()));
            } else {
    ...
    private void fillWorkflow(WorkflowInfoDO wfInfo) {
        ...
        workflowNodeInfoRepository.findByWorkflowId(wfInfo.getId()).forEach(e -> nodeIdNodInfoMap.put(e.getId(), e));
        // 填充节点信息
        if (!CollectionUtils.isEmpty(dagInfo.getNodes())) {
            for (PEWorkflowDAG.Node node : dagInfo.getNodes()) {
                ...
                if (nodeInfo != null) {
                    node.setNodeType(nodeInfo.getType()).setJobId(nodeInfo.getJobId()).setEnable(nodeInfo.getEnable())
                        .setSkipWhenFailed(nodeInfo.getSkipWhenFailed()).setNodeName(nodeInfo.getNodeName())
    ...
    private WorkflowInfoDO permissionCheck(Long wfId, Long appId) {
        WorkflowInfoDO wfInfo = workflowInfoRepository.findById(wfId)
            .orElseThrow(() -> new IllegalArgumentException("can't find workflow by id: " + wfId));
