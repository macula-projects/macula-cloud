...
public class PowerScheduleService {
        ...
        Lists.partition(appIds, MAX_APP_NUM).forEach(partAppIds -> {
            ...
            try {
                ...
                jobInfos.forEach(jobInfo -> {
                    Long instanceId =
                        instanceService.create(jobInfo.getId(), jobInfo.getAppId(), jobInfo.getJobParams(), null, null,
                            jobInfo.getNextTriggerTime()).getInstanceId();
                ...
                jobInfos.forEach(jobInfoDO -> {
                    ...
                    if (targetTriggerTime < nowTime) {
                        log.warn("[Job-{}] schedule delay, expect: {}, current: {}", jobInfoDO.getId(),
                            targetTriggerTime, System.currentTimeMillis());
                    } else {
    ...
    private void scheduleFrequentJobCore(List<Long> appIds) {
        ...
        Lists.partition(appIds, MAX_APP_NUM).forEach(partAppIds -> {
            try {
                // 查询所有的秒级任务（只包含ID）
                List<Long> jobIds = jobInfoRepository.findByAppIdInAndStatusAndTimeExpressionTypeIn(partAppIds,
                    SwitchableStatus.ENABLE.getV(), TimeExpressionType.FREQUENT_TYPES);
                if (CollectionUtils.isEmpty(jobIds)) {
                    return;
                }
                // 查询日志记录表中是否存在相关的任务
                List<Long> runningJobIdList =
                    instanceInfoRepository.findByJobIdInAndStatusIn(jobIds, InstanceStatus.GENERALIZED_RUNNING_STATUS);
                ...
                notRunningJobIds.forEach(jobId -> {
                    ...
                    jobInfoOpt.ifPresent(jobInfoDO -> {
                        ...
                        if (lifeCycle.getEnd() != null && lifeCycle.getEnd() < System.currentTimeMillis()) {
                            ...
                            jobService.runJob(jobInfoDO.getAppId(), jobId, null,
    ...
    private void refreshWorkflow(WorkflowInfoDO wfInfo) {
        ...
        Long nextTriggerTime =
            timingStrategyService.calculateNextTriggerTime(wfInfo.getNextTriggerTime(), TimeExpressionType.CRON,
                wfInfo.getTimeExpression(), lifeCycle.getStart(), lifeCycle.getEnd());
