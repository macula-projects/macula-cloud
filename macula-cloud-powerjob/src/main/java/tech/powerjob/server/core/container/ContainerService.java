...
public class ContainerService {
    ...
    public void save(ContainerInfoDO container) {
        ...
        if (originId != null) {
            // just validate
            containerInfoRepository.findById(originId)
                .orElseThrow(() -> new IllegalArgumentException("can't find container by id: " + originId));
        } else {
    ...
    public void delete(Long appId, Long containerId) {
        ...
        ContainerInfoDO container = containerInfoRepository.findById(containerId)
            .orElseThrow(() -> new IllegalArgumentException("can't find container by id: " + containerId));
    ...
    public void deploy(Long containerId, Session session) throws Exception {
        ...
        if (!lock) {
            remote.sendText(
                "SYSTEM: acquire deploy lock failed, maybe other user is deploying, please wait until the running deploy task finished.");
            return;
        }
        ...
        try {
            ...
            if (lastDeployTime != null) {
                if ((System.currentTimeMillis() - lastDeployTime.getTime()) < DEPLOY_MIN_INTERVAL) {
                    remote.sendText(
                        "SYSTEM: [warn] deploy too frequent, last deploy time is: " + DateFormatUtils.format(
                            lastDeployTime, OmsConstant.TIME_PATTERN));
                }
            }
            ...
            containerInfoRepository.saveAndFlush(container);
            remote.sendText(
                String.format("SYSTEM: update current container version=%s successfully!", container.getVersion()));
            ...
            String downloadURL =
                String.format("http://%s:%s/container/downloadJar?version=%s", NetUtils.getLocalHost(), port,
                    container.getVersion());
            ServerDeployContainerRequest req =
                new ServerDeployContainerRequest(containerId, container.getContainerName(), container.getVersion(),
                    downloadURL);
    ...
    private File prepareJarFile(ContainerInfoDO container, Session session) throws Exception {
        ...
        if (sourceType == ContainerSourceType.Git) {
            ...
            try {
                ...
                CloneCommand cloneCommand = Git.cloneRepository().setDirectory(workerDir).setURI(gitRepoInfo.getRepo())
                    .setBranch(gitRepoInfo.getBranch());
                if (!StringUtils.isEmpty(gitRepoInfo.getUsername())) {
                    CredentialsProvider credentialsProvider =
                        new UsernamePasswordCredentialsProvider(gitRepoInfo.getUsername(), gitRepoInfo.getPassword());
                ...
                if (container.getVersion().equals(oldVersion)) {
                    ...
                    remote.sendText(String.format("SYSTEM: new version detected, from %s to %s.", oldVersion,
                        container.getVersion()));
                }
                ...
                IOFileFilter fileFilter =
                    FileFilterUtils.asFileFilter((dir, name) -> name.endsWith("jar-with-dependencies.jar"));
                ...
                if (!gridFsManager.exists(GridFsManager.CONTAINER_BUCKET, jarFileName)) {
                    remote.sendText(
                        "SYSTEM: can't find the jar resource in remote, maybe this is a new version, start to upload new version.");
                    ...
                    remote.sendText(
                        "SYSTEM: find the jar resource in remote successfully, so it's no need to upload anymore.");
                }
