...
public class WorkflowDAGUtils {
     ...
     * @return 就绪的后继节点
     */
    private static List<PEWorkflowDAG.Node> moveAndObtainReadySuccessor(PEWorkflowDAG.Node skippedNode,
        Map<Long, PEWorkflowDAG.Node> nodeId2Node, Multimap<Long, Long> relyMap, Multimap<Long, Long> successorMap) {
     ...
     */
    private static boolean isReadyNode(long nodeId, Map<Long, PEWorkflowDAG.Node> nodeId2Node,
        Multimap<Long, Long> relyMap) {
        PEWorkflowDAG.Node currentNode = nodeId2Node.get(nodeId);
        int currentNodeStatus =
            currentNode.getStatus() == null ? InstanceStatus.WAITING_DISPATCH.getV() : currentNode.getStatus();
        // 跳过已完成节点（处理成功 或者 处理失败）和已派发节点（ 状态为运行中 ）
        if (InstanceStatus.FINISHED_STATUS.contains(
            currentNodeStatus) || currentNodeStatus == InstanceStatus.RUNNING.getV()) {
            return false;
        }
        ...
        for (Long relyNodeId : relyNodeIds) {
            ...
            int relyNodeStatus =
                relyNode.getStatus() == null ? InstanceStatus.WAITING_DISPATCH.getV() : relyNode.getStatus();
