...
public abstract class AbWorkerRequestHandler implements IWorkerRequestHandler {
    ...
    protected abstract AskResponse processTaskTrackerReportInstanceStatus0(TaskTrackerReportInstanceStatusReq req,
    ...
    public void processWorkerHeartbeat(WorkerHeartbeat heartbeat) {
        ...
        WorkerHeartbeatEvent event =
            new WorkerHeartbeatEvent().setAppName(heartbeat.getAppName()).setAppId(heartbeat.getAppId())
                .setVersion(heartbeat.getVersion()).setProtocol(heartbeat.getProtocol()).setTag(heartbeat.getTag())
                .setWorkerAddress(heartbeat.getWorkerAddress()).setDelayMs(startMs - heartbeat.getHeartbeatTime())
    ...
    public AskResponse processTaskTrackerReportInstanceStatus(TaskTrackerReportInstanceStatusReq req) {
        long startMs = System.currentTimeMillis();
        TtReportInstanceStatusEvent event =
            new TtReportInstanceStatusEvent().setAppId(req.getAppId()).setJobId(req.getJobId())
                .setInstanceId(req.getInstanceId()).setWfInstanceId(req.getWfInstanceId())
                .setInstanceStatus(InstanceStatus.of(req.getInstanceStatus())).setDelayMs(startMs - req.getReportTime())
    ...
    public void processWorkerLogReport(WorkerLogReportReq req) {
        ...
        WorkerLogReportEvent event = new WorkerLogReportEvent().setWorkerAddress(req.getWorkerAddress())
        ...
        if (jobInfoOpt.isPresent()) {
            ...
            if (!jobInfo.getAppId().equals(appId)) {
                ...
                List<String> sortedAvailableWorker =
                    workerClusterQueryService.getSuitableWorkers(jobInfo).stream().map(WorkerInfo::getAddress)
