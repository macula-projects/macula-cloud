...
public class WorkflowInstanceService {
    ...
    public void stopWorkflowInstance(Long wfInstanceId, Long appId) {
        ...
        dag.getNodes().forEach(node -> {
            try {
                if (node.getInstanceId() != null && InstanceStatus.GENERALIZED_RUNNING_STATUS.contains(
                    node.getStatus())) {
    ...
    /**
     * Add by Echo009 on 2021/02/20 将节点标记成功 注意：这里仅能标记真正执行失败的且不允许跳过的节点 即处于 [失败且不允许跳过] 的节点 而且仅会操作工作流实例 DAG
     * 中的节点信息（状态、result） 并不会改变对应任务实例中的任何信息
     * <p>
     * 还是加把锁保平安 ~
     *
    ...
    public void markNodeAsSuccess(Long appId, Long wfInstanceId, Long nodeId) {
        ...
        if (targetNode.getInstanceId() != null && targetNode.getStatus() == InstanceStatus.FAILED.getV()
            // 不允许失败跳过
            && !allowSkipWhenFailed) {
            // 仅处理工作流实例中的节点信息
            targetNode.setStatus(InstanceStatus.SUCCEED.getV()).setResult(SystemInstanceResult.MARK_AS_SUCCESSFUL_NODE);
