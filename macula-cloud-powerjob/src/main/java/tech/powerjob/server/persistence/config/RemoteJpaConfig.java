...
public class RemoteJpaConfig {
    ...
    /**
     * 生成配置文件，包括 JPA配置文件和Hibernate配置文件，相当于以下三个配置 spring.jpa.show-sql=false spring.jpa.open-in-view=false
    ...
    public LocalContainerEntityManagerFactoryBean initRemoteEntityManagerFactory(
        ...
        return builder.dataSource(omsRemoteDatasource).properties(datasourceProperties).packages(CORE_PACKAGES)
            .persistenceUnit("remotePersistenceUnit").build();
    }

    @Primary
    @Bean(name = "remoteTransactionManager")
    public PlatformTransactionManager initRemoteTransactionManager(@Qualifier(
        "remoteEntityManagerFactory") LocalContainerEntityManagerFactoryBean localContainerEntityManagerFactoryBean) {
