...
/**
 * 时间轮定时器 支持的最小精度：1ms（Thread.sleep本身不精确导致精度没法提高） 最小误差：1ms，理由同上
 *
 * @author tjq
 * @since 2020/4/2
 */
...
public class HashedWheelTimer implements Timer {
    ...
    public HashedWheelTimer(long tickDuration, int ticksPerWheel, int processThreadNum) {
        ...
        if (processThreadNum <= 0) {
            ...
            // 基本都是 io 密集型任务
            taskProcessPool = new ThreadPoolExecutor(core, 2 * core, 60, TimeUnit.SECONDS, queue, threadFactory,
                RejectedExecutionHandlerFactory.newCallerRun("PowerJobTimeWheelPool"));
        }
