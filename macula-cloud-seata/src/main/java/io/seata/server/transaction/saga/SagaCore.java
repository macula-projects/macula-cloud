...
public class SagaCore extends AbstractCore {
    ...
    public BranchStatus branchCommitSend(BranchCommitRequest request, GlobalSession globalSession,
        ...
        if (sagaChannel == null) {
            LOGGER.error(
                "Failed to commit SAGA global[" + globalSession.getXid() + ", cannot find channel by resourceId[" + sagaResourceId + "]");
            return BranchStatus.PhaseTwo_CommitFailed_Retryable;
        }
    ...
    public BranchStatus branchRollbackSend(BranchRollbackRequest request, GlobalSession globalSession,
        ...
        if (sagaChannel == null) {
            LOGGER.error(
                "Failed to rollback SAGA global[" + globalSession.getXid() + ", cannot find channel by resourceId[" + sagaResourceId + "]");
            return BranchStatus.PhaseTwo_RollbackFailed_Retryable;
        }
    ...
    public boolean doGlobalCommit(GlobalSession globalSession, boolean retrying) throws TransactionException {
        try {
            BranchStatus branchStatus = branchCommit(globalSession,
                SessionHelper.newBranch(BranchType.SAGA, globalSession.getXid(), -1, getSagaResourceId(globalSession),
    ...
    public void doGlobalReport(GlobalSession globalSession, String xid, GlobalStatus globalStatus)
        ...
        if (GlobalStatus.Committed.equals(globalStatus)) {
            ...
            if (GlobalStatus.RollbackRetrying.equals(globalStatus) || GlobalStatus.TimeoutRollbackRetrying.equals(
                globalStatus) || GlobalStatus.UnKnown.equals(globalStatus)) {
                globalSession.queueToRetryRollback();
