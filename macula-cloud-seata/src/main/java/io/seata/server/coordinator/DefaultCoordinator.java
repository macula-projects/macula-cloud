...
public class DefaultCoordinator extends AbstractTCInboundHandler implements TransactionMessageHandler, Disposable {
    ...
    private final GlobalStatus[] rollbackingStatuses =
        new GlobalStatus[] {GlobalStatus.TimeoutRollbacking, GlobalStatus.TimeoutRollbackRetrying,
            GlobalStatus.RollbackRetrying, GlobalStatus.Rollbacking};
    private final GlobalStatus[] retryCommittingStatuses =
        new GlobalStatus[] {GlobalStatus.Committing, GlobalStatus.CommitRetrying, GlobalStatus.Committed};
    ...
    private DefaultCoordinator(RemotingServer remotingServer) {
        ...
        if (enableBranchAsyncRemove && StoreConfig.getSessionMode() != StoreConfig.SessionMode.FILE) {
            branchRemoveExecutor =
                new ThreadPoolExecutor(BRANCH_ASYNC_POOL_SIZE, BRANCH_ASYNC_POOL_SIZE, Integer.MAX_VALUE,
    ...
    protected void doGlobalBegin(GlobalBeginRequest request, GlobalBeginResponse response, RpcContext rpcContext)
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info(
                ...
                rpcContext.getApplicationId(), rpcContext.getTransactionServiceGroup(), request.getTransactionName(),
    ...
    protected void timeoutCheck() {
        ...
        SessionHelper.forEach(beginGlobalsessions, globalSession -> {
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(
                    globalSession.getXid() + " " + globalSession.getStatus() + " " + globalSession.getBeginTime() + " " + globalSession.getTimeout());
            }
            SessionHolder.lockAndExecute(globalSession, () -> {
                ...
                LOGGER.warn(
                    ...
                    globalSession.getXid(), DateFormatUtils.ISO_DATE_FORMAT.format(globalSession.getBeginTime()),
                    DateFormatUtils.ISO_DATE_FORMAT.format(System.currentTimeMillis()));
    ...
    protected void handleRetryRollbacking() {
        ...
        SessionHelper.forEach(rollbackingSessions, rollbackingSession -> {
            try {
                ...
                LOGGER.error("Failed to retry rollbacking [{}] {} {}", rollbackingSession.getXid(), ex.getCode(),
                    ex.getMessage());
            }
        });
    }
    ...
    protected void handleRetryCommitting() {
        ...
        SessionHelper.forEach(committingSessions, committingSession -> {
            try {
                ...
                if (GlobalStatus.Committing.equals(
                    committingSession.getStatus()) && !committingSession.isDeadSession()) {
                    // The function of this 'return' is 'continue'.
                    return;
                }
                ...
                if (GlobalStatus.Committed.equals(
                    committingSession.getStatus()) && committingSession.getBranchSessions().isEmpty()) {
                ...
                LOGGER.error("Failed to retry committing [{}] {} {}", committingSession.getXid(), ex.getCode(),
                    ex.getMessage());
            }
        });
    }
    ...
    protected void undoLogDelete() {
        ...
        short saveDays =
            CONFIG.getShort(ConfigurationKeys.TRANSACTION_UNDO_LOG_SAVE_DAYS, UndoLogDeleteRequest.DEFAULT_SAVE_DAYS);
