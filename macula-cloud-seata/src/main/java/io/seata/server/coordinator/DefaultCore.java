...
public class DefaultCore implements Core {
    ...
    public DefaultCore(RemotingServer remotingServer) {
        List<AbstractCore> allCore =
            EnhancedServiceLoader.loadAll(AbstractCore.class, new Class[] {RemotingServer.class},
                ...
                try {
                    ...
                    switch (branchStatus) {
                        case PhaseTwo_Committed:
                            SessionHelper.removeBranch(globalSession, branchSession, !retrying);
                            LOGGER.info("Commit branch transaction successfully, xid = {} branchId = {}",
                                globalSession.getXid(), branchSession.getBranchId());
                            return CONTINUE;
                ...
                try {
                    ...
                    switch (branchStatus) {
                        ...
                        case PhaseTwo_RollbackFailed_Unretryable:
                            ...
                            LOGGER.error("Rollback branch transaction fail and stop retry, xid = {} branchId = {}",
                                globalSession.getXid(), branchSession.getBranchId());
                            return false;
                        default:
                            LOGGER.error("Rollback branch transaction fail and will retry, xid = {} branchId = {}",
                                globalSession.getXid(), branchSession.getBranchId());
                            if (!retrying) {
    ...
    private boolean isXaerNotaTimeout(GlobalSession globalSession, BranchStatus branchStatus) {
        if (BranchStatus.PhaseTwo_CommitFailed_XAER_NOTA_Retryable.equals(
            branchStatus) || BranchStatus.PhaseTwo_RollbackFailed_XAER_NOTA_Retryable.equals(branchStatus)) {
            return System.currentTimeMillis() > globalSession.getBeginTime() + globalSession.getTimeout() + Math.max(
                RETRY_XAER_NOTA_TIMEOUT, globalSession.getTimeout());
        } else {
            return false;
        }
    }
