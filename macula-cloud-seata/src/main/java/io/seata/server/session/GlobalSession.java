...
public class GlobalSession implements SessionLifecycle, SessionStorable {
    ...
    /**
     * If the global session's status is (Rollbacking or Committing) and currentTime - createTime >=
     * RETRY_DEAD_THRESHOLD then the tx will be remand as need to retry rollback
     */
    ...
    public boolean isSaga() {
        ...
        if (branchSessions.size() > 0) {
            ...
            return StringUtils.isNotBlank(transactionName) && transactionName.startsWith(
                Constants.SAGA_TRANS_NAME_PREFIX);
        }
    }
    ...
    public byte[] encode() {
        ...
        if (size > MAX_GLOBAL_SESSION_SIZE) {
            throw new RuntimeException(
                "global session size exceeded, size : " + size + " maxBranchSessionSize : " + MAX_GLOBAL_SESSION_SIZE);
        }
    ...
    private int calGlobalSessionSize(byte[] byApplicationIdBytes, byte[] byServiceGroupBytes, byte[] byTxNameBytes,
        ...
        final int size = 8 // transactionId
            ...
            + 8 // beginTime
            + 1 // statusCode
            + (byApplicationIdBytes == null ? 0 : byApplicationIdBytes.length) + (byServiceGroupBytes == null ? 0
            : byServiceGroupBytes.length) + (byTxNameBytes == null ? 0 : byTxNameBytes.length) + (xidBytes == null ? 0
            : xidBytes.length) + (applicationDataBytes == null ? 0 : applicationDataBytes.length);
        return size;
    }
    ...
    public String toString() {
        return "GlobalSession{" + "xid='" + xid + '\'' + ", transactionId=" + transactionId + ", status=" + status + ", applicationId='" + applicationId + '\'' + ", transactionServiceGroup='" + transactionServiceGroup + '\'' + ", transactionName='" + transactionName + '\'' + ", timeout=" + timeout + ", beginTime=" + beginTime + ", applicationData='" + applicationData + '\'' + ", lazyLoadBranch=" + lazyLoadBranch + ", active=" + active + ", branchSessions=" + branchSessions + ", globalSessionLock=" + globalSessionLock + ", lifecycleListeners=" + lifecycleListeners + '}';
    }
