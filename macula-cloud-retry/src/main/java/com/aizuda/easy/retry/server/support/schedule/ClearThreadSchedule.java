...
public class ClearThreadSchedule {
    ...
    public void clearOfflineNode() {
        ...
        try {
            ...
            Set<RegisterNodeInfo> waitOffline =
                allPods.stream().filter(registerNodeInfo -> registerNodeInfo.getExpireAt().isBefore(endTime))
    ...
    public void clearLog() {
        try {
            ...
            retryTaskLogMessageMapper.delete(
                new LambdaUpdateWrapper<RetryTaskLogMessage>().le(RetryTaskLogMessage::getCreateDt, endTime));
        } catch (Exception e) {
