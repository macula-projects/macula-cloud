...
public class DashBoardServiceImpl implements DashBoardService {
    ...
    public TaskQuantityResponseVO countTask() {
        ...
        taskQuantityResponseVO.setMaxRetryCount(
            retryTaskLogMapper.countTaskByRetryStatus(RetryStatusEnum.MAX_COUNT.getStatus()));
        taskQuantityResponseVO.setRunning(
            taskQuantityResponseVO.getTotal() - taskQuantityResponseVO.getFinish() - taskQuantityResponseVO.getMaxRetryCount());
    ...
    public DispatchQuantityResponseVO countDispatch() {
        ...
        Long success = retryTaskLogMapper.selectCount(
            new LambdaQueryWrapper<RetryTaskLog>().eq(RetryTaskLog::getRetryStatus,
                RetryStatusEnum.FINISH.getStatus()));
        dispatchQuantityResponseVO.setSuccessPercent(
            BigDecimal.valueOf(success).divide(BigDecimal.valueOf(total), 2, RoundingMode.HALF_UP)
    ...
    public ActivePodQuantityResponseVO countActivePod() {
        ...
        activePodQuantityResponseVO.setServerTotal(serverNodeMapper.selectCount(
            new LambdaQueryWrapper<ServerNode>().eq(ServerNode::getNodeType, NodeTypeEnum.SERVER.getType())));
        activePodQuantityResponseVO.setClientTotal(serverNodeMapper.selectCount(
            new LambdaQueryWrapper<ServerNode>().eq(ServerNode::getNodeType, NodeTypeEnum.CLIENT.getType())));

        return activePodQuantityResponseVO;
    }

    @Override
    public List<SceneQuantityRankResponseVO> rankSceneQuantity(String groupName, String type, String startTime,
        String endTime) {
        LocalDateTime startDateTime = LocalDateTime.now();
        if (StringUtils.isNotBlank(startTime)) {
            startDateTime =
                LocalDateTime.of(LocalDate.parse(startTime, DateTimeFormatter.ofPattern("yyyy-MM-dd")), LocalTime.MIN);
        }

        LocalDateTime endDateTime = LocalDateTime.now();
        if (StringUtils.isNotBlank(endTime)) {
            endDateTime =
                LocalDateTime.of(LocalDate.parse(endTime, DateTimeFormatter.ofPattern("yyyy-MM-dd")), LocalTime.MAX);
        }
    ...
    @Override
    public List<DispatchQuantityResponseVO> lineDispatchQuantity(String groupName, String type, String startTime,
        String endTime) {
        ...
        List<DispatchQuantityResponseVO> totalDispatchQuantityResponseList =
            retryTaskLogMapper.lineDispatchQuantity(groupName, null, type, startDateTime, endDateTime);

        List<DispatchQuantityResponseVO> successDispatchQuantityResponseList =
            retryTaskLogMapper.lineDispatchQuantity(groupName, RetryStatusEnum.FINISH.getStatus(), type, startDateTime,
                endDateTime);
        Map<String, DispatchQuantityResponseVO> successDispatchQuantityResponseVOMap =
            successDispatchQuantityResponseList.stream()
                .collect(Collectors.toMap(DispatchQuantityResponseVO::getCreateDt, i -> i));
        for (DispatchQuantityResponseVO dispatchQuantityResponseVO : totalDispatchQuantityResponseList) {

            DispatchQuantityResponseVO quantityResponseVO =
                successDispatchQuantityResponseVOMap.get(dispatchQuantityResponseVO.getCreateDt());
            ...
            dispatchQuantityResponseVO.setFail(
                dispatchQuantityResponseVO.getTotal() - dispatchQuantityResponseVO.getSuccess());

        }

        dateTypeEnum.getConsumer().accept(totalDispatchQuantityResponseList);

        return totalDispatchQuantityResponseList.stream()
            .sorted(Comparator.comparing(DispatchQuantityResponseVO::getCreateDt)).collect(Collectors.toList());
    }
    ...
    public PageResult<List<ServerNodeResponseVO>> pods(ServerNodeQueryVO queryVO) {
        ...
        PageDTO<ServerNode> serverNodePageDTO =
            serverNodeMapper.selectPage(pageDTO, serverNodeLambdaQueryWrapper.orderByDesc(ServerNode::getNodeType));

        List<ServerNodeResponseVO> serverNodeResponseVOS =
            ServerNodeResponseVOConverter.INSTANCE.toServerNodeResponseVO(serverNodePageDTO.getRecords());
        ...
        for (final ServerNodeResponseVO serverNodeResponseVO : serverNodeResponseVOS) {
            ...
            ServerNodeExtAttrs serverNodeExtAttrs =
                JsonUtil.parseObject(serverNodeResponseVO.getExtAttrs(), ServerNodeExtAttrs.class);

            try {

                // 从远程节点取
                String format = MessageFormat.format(URL, serverNodeResponseVO.getHostIp(),
                    serverNodeExtAttrs.getWebPort().toString());
                ...
                LogUtils.error(log, "Failed to retrieve consumer group for node [{}:{}].",
                    serverNodeResponseVO.getHostIp(), serverNodeExtAttrs.getWebPort());
