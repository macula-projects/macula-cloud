...
public class AlarmNotifyThreadSchedule {
    ...
    public void retryTaskMoreThreshold() {
        ...
        for (GroupConfig groupConfig : configAccess.getAllConfigGroupList()) {
            List<NotifyConfig> notifyConfigs = configAccess.getNotifyConfigByGroupName(groupConfig.getGroupName(),
                NotifySceneEnum.MAX_RETRY.getNotifyScene());
            if (CollectionUtils.isEmpty(notifyConfigs)) {
                continue;
            }

            int count = retryTaskMapper.countAllRetryTaskByRetryStatus(groupConfig.getGroupPartition(),
                ...
                if (count > notifyConfig.getNotifyThreshold()) {
                    ...
                    AlarmContext context = AlarmContext.build()
                        .text(retryTaskMoreThresholdTextMessageFormatter, EnvironmentUtils.getActiveProfile(),
                            groupConfig.getGroupName(), LocalDateTime.now().format(formatter), count)
    ...
    public void retryErrorMoreThreshold() {
        ...
        for (GroupConfig groupConfig : configAccess.getAllConfigGroupList()) {
            List<NotifyConfig> notifyConfigs = configAccess.getNotifyConfigByGroupName(groupConfig.getGroupName(),
                NotifySceneEnum.MAX_RETRY_ERROR.getNotifyScene());
            if (CollectionUtils.isEmpty(notifyConfigs)) {
                continue;
            }
            ...
            int count = retryDeadLetterMapper.countRetryDeadLetterByCreateAt(now.minusMinutes(30), now,
            ...
            for (NotifyConfig notifyConfig : notifyConfigs) {
                if (count > notifyConfig.getNotifyThreshold()) {
                    // 预警
                    AlarmContext context = AlarmContext.build()
                        .text(retryErrorMoreThresholdTextMessageFormatter, EnvironmentUtils.getActiveProfile(),
                            groupConfig.getGroupName(), now.minusMinutes(30).format(formatter), now.format(formatter),
                            count).title("组:[{}] 环境重试失败数据监控", groupConfig.getGroupName())
                        .notifyAttribute(notifyConfig.getNotifyAttribute());
