...
public class RetryDeadLetterController {
    ...
    public boolean rollback(@RequestParam("groupName") String groupName, @PathVariable("id") Long id) {
        return retryDeadLetterService.rollback(groupName, id);
    }

    @LoginRequired
    @DeleteMapping("/{id}")
    public boolean deleteById(@RequestParam("groupName") String groupName, @PathVariable("id") Long id) {
