...
public class LogActor extends AbstractActor {
    ...
    public Receive createReceive() {
        return receiveBuilder().match(RetryTaskLogDTO.class,
            ...
            }).match(RetryTaskLogDTO.class, (retryTaskLogDTO) -> RetryStatusEnum.MAX_COUNT.getStatus()
            .equals(retryTaskLogDTO.getRetryStatus()) || RetryStatusEnum.FINISH.getStatus()
            .equals(retryTaskLogDTO.getRetryStatus()), retryTaskLogDTO -> {

            // 变动日志的状态
            RetryTaskLog retryTaskLog = new RetryTaskLog();
            retryTaskLog.setRetryStatus(retryTaskLogDTO.getRetryStatus());
            retryTaskLogMapper.update(retryTaskLog,
                new LambdaUpdateWrapper<RetryTaskLog>().eq(RetryTaskLog::getUniqueId, retryTaskLogDTO.getUniqueId())
    ...
    private void saveRetryTaskLogMessage(final RetryTaskLogDTO retryTaskLogDTO) {
        ...
        retryTaskLogMessage.setMessage(StringUtils.isBlank(errorMessage) ? StringUtils.EMPTY : errorMessage);
