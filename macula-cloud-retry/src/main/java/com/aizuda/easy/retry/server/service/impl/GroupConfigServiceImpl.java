...
public class GroupConfigServiceImpl implements GroupConfigService {
    ...
    public Boolean addGroup(GroupConfigRequestVO groupConfigRequestVO) {
        ...
        Assert.isTrue(groupConfigMapper.selectCount(new LambdaQueryWrapper<GroupConfig>().eq(GroupConfig::getGroupName,
    ...
    public Boolean updateGroup(GroupConfigRequestVO groupConfigRequestVO) {
        ...
        Assert.isTrue(totalPartition > groupConfigRequestVO.getGroupPartition(),
            () -> new EasyRetryServerException("分区超过最大分区. [{}]", totalPartition - 1));
    ...
    public Boolean updateGroupStatus(String groupName, Integer status) {
        ...
        return groupConfigMapper.update(groupConfig,
            new LambdaUpdateWrapper<GroupConfig>().eq(GroupConfig::getGroupName, groupName)) == 1;
    }
    ...
    public PageResult<List<GroupConfigResponseVO>> getGroupConfigForPage(GroupConfigQueryVO queryVO) {
        ...
        PageDTO<GroupConfig> groupConfigPageDTO =
            groupConfigMapper.selectPage(new PageDTO<>(queryVO.getPage(), queryVO.getSize()),
                groupConfigLambdaQueryWrapper);
        ...
        PageResult<List<GroupConfigResponseVO>> pageResult =
            new PageResult<>(groupConfigPageDTO.getCurrent(), groupConfigPageDTO.getSize(),
                groupConfigPageDTO.getTotal());

        List<GroupConfigResponseVO> responseVOList =
            GroupConfigResponseVOConverter.INSTANCE.toGroupConfigResponseVO(records);
    ...
    private void doSaveGroupConfig(GroupConfigRequestVO groupConfigRequestVO) {
        ...
        Assert.isTrue(1 == groupConfigMapper.insert(groupConfig),
    ...
    public GroupConfigResponseVO getGroupConfigByGroupName(String groupName) {
        GroupConfig groupConfig =
            groupConfigMapper.selectOne(new LambdaQueryWrapper<GroupConfig>().eq(GroupConfig::getGroupName, groupName));

        GroupConfigResponseVO groupConfigResponseVO =
            GroupConfigResponseVOConverter.INSTANCE.toGroupConfigResponseVO(groupConfig);
    ...
    public List<String> getAllGroupNameList() {
        return groupConfigMapper.selectList(new LambdaQueryWrapper<GroupConfig>().select(GroupConfig::getGroupName))
    ...
    private void doSaveNotifyConfig(String groupName, GroupConfigRequestVO.NotifyConfigVO notifyConfigVO) {
        ...
        Assert.isTrue(1 == notifyConfigMapper.insert(notifyConfig),
            () -> new EasyRetryServerException("failed to insert notify. sceneConfig:[{}]",
    ...
    private void doUpdateSceneConfig(GroupConfigRequestVO groupConfigRequestVO) {
        ...
        if (!CollectionUtils.isEmpty(sceneList)) {
            ...
            List<SceneConfig> sceneConfigs = sceneConfigMapper.selectList(
                new LambdaQueryWrapper<SceneConfig>().eq(SceneConfig::getGroupName,
                    groupConfigRequestVO.getGroupName()));
            Map<String, SceneConfig> sceneConfigMap =
                sceneConfigs.stream().collect(Collectors.toMap(SceneConfig::getSceneName, i -> i));
            ...
            while (iterator.hasNext()) {
                ...
                if (Objects.isNull(oldSceneConfig)) {
                    ...
                    Assert.isTrue(1 == sceneConfigMapper.deleteById(oldSceneConfig.getId()),
                        () -> new EasyRetryServerException("failed to delete scene. [{}]",
                            sceneConfigVO.getSceneName()));
                } else {
                    SceneConfig sceneConfig = SceneConfigConverter.INSTANCE.convert(sceneConfigVO);
                    sceneConfig.setGroupName(groupConfigRequestVO.getGroupName());

                    Assert.isTrue(1 == sceneConfigMapper.update(sceneConfig,
                            new LambdaQueryWrapper<SceneConfig>().eq(SceneConfig::getGroupName, sceneConfig.getGroupName())
        ...
        for (GroupConfigRequestVO.NotifyConfigVO notifyConfigVO : notifyList) {
            ...
            if (Objects.isNull(notifyConfigVO.getId())) {
                ...
                Assert.isTrue(1 == notifyConfigMapper.deleteById(notifyConfigVO.getId()),
                    () -> new EasyRetryServerException("failed to delete notify. sceneConfig:[{}]",
                        JsonUtil.toJsonString(notifyConfigVO)));
            } else {
                // update
                Assert.isTrue(1 == notifyConfigMapper.update(notifyConfig,
                        new LambdaQueryWrapper<NotifyConfig>().eq(NotifyConfig::getId, notifyConfigVO.getId())
                            .eq(NotifyConfig::getGroupName, notifyConfig.getGroupName())
                            .eq(NotifyConfig::getNotifyScene, notifyConfig.getNotifyScene())),
                    () -> new EasyRetryServerException("failed to update notify. sceneConfig:[{}]",
                        JsonUtil.toJsonString(notifyConfig)));
            }
        }
    }
