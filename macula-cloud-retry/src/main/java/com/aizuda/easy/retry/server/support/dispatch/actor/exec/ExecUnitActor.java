...
public class ExecUnitActor extends AbstractActor {
    ...
    public Receive createReceive() {
        return receiveBuilder().match(RetryExecutor.class, retryExecutor -> {
            ...
            MaxAttemptsPersistenceRetryContext context =
                (MaxAttemptsPersistenceRetryContext)retryExecutor.getRetryContext();
            ...
            try {
                ...
                if (Objects.nonNull(serverNode)) {
                    ...
                    retryExecutor.call((Callable<Result<DispatchRetryResultDTO>>)() -> {
                        ...
                        // 回调接口请求成功，处理返回值
                        if (StatusEnum.YES.getStatus() != result.getStatus() && StringUtils.isNotBlank(
                            result.getMessage())) {
                            retryTaskLog.setMessage(result.getMessage());
                        } else {
                            DispatchRetryResultDTO data = JsonUtil.parseObject(JsonUtil.toJsonString(result.getData()),
                                DispatchRetryResultDTO.class);
                            result.setData(data);
    ...
    private Result<DispatchRetryResultDTO> callClient(RetryTask retryTask, RegisterNodeInfo serverNode) {
        ...
        RpcClient rpcClient = RequestBuilder.<RpcClient, Result>newBuilder().hostPort(serverNode.getHostPort())
            .groupName(serverNode.getGroupName()).hostId(serverNode.getHostId()).hostIp(serverNode.getHostIp())
            .contextPath(serverNode.getContextPath()).client(RpcClient.class).build();
