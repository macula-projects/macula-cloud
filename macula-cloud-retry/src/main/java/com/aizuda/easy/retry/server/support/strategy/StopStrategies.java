...
public class StopStrategies {
     ...
     * 根据客户端返回结果集判断是否应该停止
     * <p>
     * 1、{@link Result#getStatus()} 不为1 则继续重试 2、根据{@link Result#getData()}中的statusCode判断是否停止
     */
    private static final class ResultStatusCodeStopStrategy implements StopStrategy {
        ...
        public boolean shouldStop(RetryContext retryContext) {
            ...
            return RetryResultStatusEnum.SUCCESS.getStatus().equals(status) || RetryResultStatusEnum.STOP.getStatus()
                .equals(status);
        }
