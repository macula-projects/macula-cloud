...
public enum DateTypeEnum {
    ...
    DAY(dispatchQuantityResponseVOList -> {
        Map<String, DispatchQuantityResponseVO> dispatchQuantityResponseVOMap = dispatchQuantityResponseVOList.stream()
    ...
    WEEK(dispatchQuantityResponseVOList -> {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            String format = LocalDateTime.of(LocalDate.now().with(DayOfWeek.of(1)), LocalTime.MIN).plusDays(i)
    ...
    MONTH(dispatchQuantityResponseVOList -> {
        ...
        for (int i = 0; i < curMonth; i++) {
            ...
            DateTimeFormatter dateTimeFormatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            String format =
                LocalDateTime.of(LocalDate.now().with(TemporalAdjusters.firstDayOfMonth()), LocalTime.MIN).plusDays(i)
                    .format(dateTimeFormatter);
            DispatchQuantityResponseVO dispatchQuantityResponseVO = dispatchQuantityResponseVOMap.get(format);
            if (Objects.isNull(dispatchQuantityResponseVO)) {
                dispatchQuantityResponseVO = new DispatchQuantityResponseVO();
    ...
    YEAR(dispatchQuantityResponseVOList -> {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            String format =
                LocalDateTime.of(LocalDate.now().with(TemporalAdjusters.firstDayOfYear()), LocalTime.MIN).plusMonths(i)
                    .format(dateTimeFormatter);
            DispatchQuantityResponseVO dispatchQuantityResponseVO = dispatchQuantityResponseVOMap.get(format);
            if (Objects.isNull(dispatchQuantityResponseVO)) {
                dispatchQuantityResponseVO = new DispatchQuantityResponseVO();
    ...
    DateTypeEnum(Consumer<List<DispatchQuantityResponseVO>> listConsumer,
        Function<LocalDateTime, LocalDateTime> startTime, Function<LocalDateTime, LocalDateTime> endTime) {
