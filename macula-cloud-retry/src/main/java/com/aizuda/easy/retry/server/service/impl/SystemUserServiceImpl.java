...
public class SystemUserServiceImpl implements SystemUserService {
    ...
    public SystemUserResponseVO login(SystemUserRequestVO requestVO) {
        ...
        List<SystemUserPermission> systemUserPermissions = systemUserPermissionMapper.selectList(
            new LambdaQueryWrapper<SystemUserPermission>().eq(SystemUserPermission::getSystemUserId,
                systemUser.getId()));
        systemUserResponseVO.setGroupNameList(
            systemUserPermissions.stream().map(SystemUserPermission::getGroupName).collect(Collectors.toList()));
    ...
    public void update(SystemUserRequestVO requestVO) {
        SystemUser systemUser =
            systemUserMapper.selectOne(new LambdaQueryWrapper<SystemUser>().eq(SystemUser::getId, requestVO.getId()));
        if (Objects.isNull(systemUser)) {
    ...
    public PageResult<List<SystemUserResponseVO>> getSystemUserPageList(SystemUserQueryVO queryVO) {
        ...
        userPageDTO =
            systemUserMapper.selectPage(userPageDTO, systemUserLambdaQueryWrapper.orderByDesc(SystemUser::getId));

        List<SystemUserResponseVO> userResponseVOList =
            SystemUserResponseVOConverter.INSTANCE.batchConvert(userPageDTO.getRecords());
        ...
        userResponseVOList.stream()
            ...
            .forEach(systemUserResponseVO -> {
                List<SystemUserPermission> systemUserPermissionList = systemUserPermissionMapper.selectList(
                    new LambdaQueryWrapper<SystemUserPermission>().select(SystemUserPermission::getGroupName)
                        .eq(SystemUserPermission::getSystemUserId, systemUserResponseVO.getId()));

                systemUserResponseVO.setGroupNameList(
                    systemUserPermissionList.stream().map(SystemUserPermission::getGroupName)
    ...
    public SystemUserResponseVO getSystemUserByUserName(String username) {
        SystemUser systemUser =
            systemUserMapper.selectOne(new LambdaQueryWrapper<SystemUser>().eq(SystemUser::getUsername, username));
        if (Objects.isNull(systemUser)) {
