<template>
	<el-dropdown class="user panel-item" trigger="click" @command="changeTenant">
		<div class="user-avatar"> 
			...
			<el-icon v-if="tenantOptions.length>1" class="el-icon--right"><el-icon-arrow-down /></el-icon>
		</div>
		<template #dropdown>
			<el-dropdown-menu>
				<el-dropdown-item :divided="0 != index" v-for="(tenantOption, index) in tenantOptions" :key="index" :command="tenantOption">{{ tenantOption.label }}</el-dropdown-item>
			</el-dropdown-menu>
		</template>
	</el-dropdown>
</template>
